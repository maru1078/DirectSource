...
void SkinnedMeshShader::load(const std::string & fileName)
{
	...
	m_device->CreateInputLayout(layout, 5, compileVs->GetBufferPointer(), compileVs->GetBufferSize(), &m_vertexLayout);
	compileVs->Release();
}

void SkinnedMeshShader::world(const XMFLOAT4X4 & mat)
{
	m_data.world = mat;
}

void SkinnedMeshShader::view(const XMFLOAT4X4 & mat)
{
	m_data.view = mat;
}

void SkinnedMeshShader::projection(const XMFLOAT4X4 & mat)
{
...
void SkinnedMeshShader::boneMatrices(int size, const XMFLOAT4X4 matrices[])
{
