...
SimpleMeshShader::SimpleMeshShader(ID3D11Device * device, ID3D11DeviceContext * deviceContext)
	: m_device{ device }
...
SimpleMeshShader::~SimpleMeshShader()
{
	clear();
}

void SimpleMeshShader::load(const std::string & fileName)
{
	...
	m_device->CreateInputLayout(layout, 3, compileVs->GetBufferPointer(), compileVs->GetBufferSize(), &m_vertexLayout);
	compileVs->Release();
}

void SimpleMeshShader::world(const XMMATRIX & mat)
{
	m_data.world = mat;
}

void SimpleMeshShader::view(const XMMATRIX & mat)
{
	m_data.view = mat;
}

void SimpleMeshShader::projection(const XMMATRIX & mat)
{
	m_data.projection = mat;
}

void SimpleMeshShader::clear()
{
...
void SimpleMeshShader::end()
{
}

void SimpleMeshShader::material(const SimpleMesh::Material & material)
{
